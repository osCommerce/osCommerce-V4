<div class="or_box_head">{$smarty.const.TEXT_CHANGE_KEY}</div>
<form name="texts" action="" method="post" id="texts_change_entity" onSubmit="return changesApply();">
    <div class="col_desc">{$smarty.const.TABLE_HEADING_LANGUAGE_KEY}</div>        
    <div class="col_desc">{tep_draw_input_field('new_key', '')}</div>
