...
{call renderAclTreeFull items=$aclTree}
    ...
    {Html::input('hidden', 'department_id', $department_id)}
</form>
<script>
function recalcAcl(obj) {
    ...
    //return false;
    $.post("{$app->urlManager->createUrl('departments-adminmembers/recalc-acl')}", $('#save_item_form').serialize(), function (data, status) {
        if (status == "success") {
            $('#acl_box').html(data);
        } else {
            alert("Request error.");
...
function saveItem() {
    $.post("{$app->urlManager->createUrl('departments-adminmembers/submit-permissions')}", $('#save_item_form').serialize(), function (data, status) {
        if (status == "success") {
