...
function recalcAcl(obj) {
    ...
    return false;
    $.post("{$app->urlManager->createUrl('departments-adminfiles/recalc-acl')}", $('#save_item_form').serialize(), function (data, status) {
        if (status == "success") {
            $('#acl_box').html(data);
        } else {
            alert("Request error.");
...
function saveItem() {
    $.post("{$app->urlManager->createUrl('departments-adminfiles/submit')}", $('#save_item_form').serialize(), function (data, status) {
        if (status == "success") {
