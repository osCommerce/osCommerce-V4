{tep_draw_form('status', 'orders_products_status/save', 'orders_products_status_id='|cat:$orders_products_status_id)}
    ...
    {/if}
        {foreach $languages as $lKey => $lItem}
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    {$orders_products_status_inputs_string_long[$lItem.id]}
                </div>
            </div>
            <div class="row">
                <div class="col-md-2"><label>{$smarty.const.TEXT_INFO_ORDERS_PRODUCTS_STATUS_NAME}</label></div>
                <div class="col-md-4">
                    {$orders_products_status_inputs_string[$lItem.id]}
                </div>
            </div>
    ...
    {foreach $orders_products_status_manual_matrix_string as $opsmArray}
        <div class="row">
            <div class="col-md-2">{$opsmArray['label']}</div>
            <div class="col-md-4">{$opsmArray['element']}</div>
        </div>
    {/foreach}
    <div class="btn-bar" style="padding: 0;">
        <div class="btn-left">
            <a href="{Yii::$app->urlManager->createUrl(['orders_products_status', 'orders_products_status_id' => $orders_products_status_id])}" class="btn btn-cancel">{$smarty.const.IMAGE_CANCEL}</a>
        </div>
        <div class="btn-right">
    ...
    $(function() {
        $('.btn-confirm').on('click', (function(e) {
            ...
            $.post(action, data, function(response) {
                ...
                if (response.added) {
                    window.location.href = "orders_products_status/edit?orders_products_status_id=" + response.added;
                }
                setTimeout(function() {
