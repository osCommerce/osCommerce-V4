...
export default function exportBlock(){
    ...
    const placeholder = $(this).closest('.box').closest('div[data-placeholder]').data('placeholder');
    ...
    const $form = $(`
            <form>
                ...
                <div class="row align-items-center m-b-2 group-categories">
                    ...
                    <div class="col-7">
                        ${groupCategories(pageName, placeholder).html()}
                    </div>
                </div>
                <div class="row m-b-2">
                    <div class="col-5 align-right">
...
function groupCategories(pageName, placeholder) {
    const $html = $(`<div><select name="group-categories" class="form-control">
                            <option value=""></option>
                            <option value="header"${pageName == 'header' && placeholder != 'header-menu' ? ' selected' : ''}>Header</option>
                            <option value="footer"${pageName == 'footer' ? ' selected' : ''}>Footer</option>
                            <option value="header-menu"${placeholder == 'header-menu' ? ' selected' : ''}>Header menu</option>
                        </select></div>`);
    ...
    for (let name in entryData.pages) {
        ...
        $pageName.append(`<option value="${entryData.pages[name].page_name}"
            ${pageName == entryData.pages[name].page_name ? ' selected' : ''}>
                ${entryData.pages[name].title}
            </option>`);
    }
